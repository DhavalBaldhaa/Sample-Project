...
object TextUtil {
    ...
    fun isNullOrEmpty(textView: TextView?): Boolean {
        ...
        return if (textView.text == null) true else isNullOrEmpty(textView.text.toString().trim { it <= ' ' })
    }

    fun isNullOrEmpty(editText: EditText?): Boolean {
        if (editText == null) return true
        return if (editText.text == null) true else isNullOrEmpty(editText.text.toString().trim { it <= ' ' })
    }
